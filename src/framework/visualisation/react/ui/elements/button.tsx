...
export const PrimaryButton = ({
  ...
  onClick
}: Weak<PropsUIButtonPrimary>): JSX.Element => {
  return (
    <div className='relative min-w-button'>
      <div
        className={`flex flex-col items-center leading-none font-button text-button rounded ${
          enabled ? 'cursor-pointer active:shadow-top4px' : ''
        } ${color}`}
        onClick={onClick}
      >
        <div
...
export const SecondaryButton = ({
  ...
  return (
    <div className='relative min-w-button'>
      <div
        className={`flex flex-col items-center active:shadow-top2px border-2 font-button text-button rounded bg-opacity-0 cursor-pointer ${color}`}
        onClick={onClick}
      >
        <div className='pt-13px pb-13px pr-4 pl-4 active:pt-14px active:pb-3'>{label}</div>
      </div>
    </div>
  )
}
      ...
      <div className='flex items-center'>
        ...
        <div className='focus:outline-none'>
          <div className='flex flex-col justify-center h-full items-center'>
            <div className={`flex-wrap text-button font-button ${color}`}>{label}</div>
          </div>
        </div>
        <div className={`${alignment !== 'left' ? '' : 'hidden'}`}>
